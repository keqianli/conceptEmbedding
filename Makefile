export CC  = gcc
export CXX = g++
export CFLAGS = -std=c++11 -Wall -msse2 -fopenmp -I./src/ -g

BIN = build/silhouetting build/batch_encode build/batch_parse build/batch_segment build/web_query
TEST = build/test_model build/test_inference build/test_print_silhouette build/test_parser build/batch_test_inference
...
build/batch_test_inference: build/inference_online.o src/test/test_inference.cpp build/model.o
	$(CXX) $(CFLAGS) -pg src/test/batch_test_inference.cpp src/online/encoder.cpp build/*.o -o $@
